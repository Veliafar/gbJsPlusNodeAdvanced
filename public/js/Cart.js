const cart = {
  template: `   
   <div>    
      <li class="header__menu-list__item cart-button" @click="cartClick()">
  ...
  methods: {
    ...
    addItemToCart(item) {
      ...
      if (cartItem) {
        ...
        this.$root.putJson(`/api/cart/${ item.id_product }/${ item.product_name }`, { quantity: 1 })
          .then(data => {
            if (data.result) {
              cartItem.quantity++;
              this.mathTotal();
        ...
        this.$root.postJson(`api/cart/${ item.id_product }/${ item.product_name }`, cartItem)
          .then(data => {
            if (data.result) {
    ...
    removeItem(item) {
      this.$root.delJson(`/api/cart/${ item.id_product }/${ item.product_name }`, item)
        .then(data => {
          if (data.result) {
