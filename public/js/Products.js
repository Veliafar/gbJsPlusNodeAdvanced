const products = {
  template: `
    ...
    <section class="container goods">
      ...
      <ul class="goods__list">
        ...
        <li class="goods__item-wrapper" v-for="item of itemListFiltered">
        <article class="goods__item">
          <div class="goods__img-wrapper">
            <img class="goods__img-wrapper__img" :src="'./img/goods/' + item.imgProduct" :alt="item.product_name">
            <div class="goods__img-wrapper__overlay">
              <div class="goods__img-wrapper__overlay__text">
                <a  class="button" @click="$root?.$refs?.headerComp?.$refs?.cart?.addItemToCart(item)">
  ...
  mounted() {
    this.$parent.getJson('/api/products')
      .then(data => {
        this.itemList = Array.from([...data])
        this.itemListFiltered = Array.from([...data]);
      })
  },
  methods: {
